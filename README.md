<div align="center">

# Subham Bhattacharya

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=22&duration=3200&pause=900&color=7AA2F7&center=true&vCenter=true&width=640&lines=Software+Engineer+%C2%B7+Backend+%26+Python;Building+reliable%2C+well-structured+systems;Strong+CS+foundations+%C2%B7+Toward+AI+engineering" alt="Subham Bhattacharya — Backend Engineer" />

<p>
  <img src="https://img.shields.io/badge/Focus-Backend_Engineering-7AA2F7?style=flat-square&labelColor=1A1B27" alt="Focus" />
  <img src="https://img.shields.io/badge/Currently-Python_%C2%B7_DSA_%C2%B7_AI-7AA2F7?style=flat-square&labelColor=1A1B27" alt="Currently learning" />
  <img src="https://img.shields.io/badge/Based_in-India-7AA2F7?style=flat-square&labelColor=1A1B27" alt="Location" />
</p>

</div>

---

## About

I'm an entrepreneur moving deliberately into software engineering, focused on backend development and the fundamentals that make systems dependable. Running real operations taught me to think in constraints, edge cases, and what actually has to hold up in production — and I bring that same mindset to code.

Right now I'm building depth in **Python, data structures, and algorithms** through structured study and hands-on projects, working toward remote backend roles and, longer term, AI/ML systems engineering. I value clarity, correctness, and maintainability over cleverness.

- Backend-focused, Python-first
- Comfortable reasoning about data, APIs, and clean architecture
- Learning in public, building consistently

**Email** — subham.bh@icloud.com

---

## Tech Stack

**Languages**

<p>
  <img src="https://skillicons.dev/icons?i=py,c,cpp,js,html,css&theme=dark" alt="Languages" />
</p>

**Backend & Data**

<p>
  <img src="https://skillicons.dev/icons?i=flask,django,sqlite&theme=dark" alt="Backend and data" />
</p>

**Tooling**

<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,pycharm,linux&theme=dark" alt="Tools" />
</p>

---

## Core Competencies

**Programming & Computer Science**
- Python (intermediate) — OOP, functions, modules, error handling
- Data structures — lists, dictionaries, tuples, sets
- Algorithmic problem solving and clean control flow
- Regular expressions and text processing

**Backend & Data**
- REST APIs, JSON, CSV, and HTTP requests
- Relational database fundamentals (SQL, SQLite)
- Data extraction and processing from the web

**Engineering Practice**
- Git and GitHub for version control and collaboration
- Command-line workflows, debugging, and refactoring
- Writing readable, structured, maintainable code

**Foundations & Tools**
- HTML, CSS, and JavaScript basics
- Microsoft Excel for data handling and analysis
- Canva, WordPress, and Wix for design and web publishing

---

## Currently Building

- Strengthening **data structures & algorithms** for technical interviews
- Building **REST APIs** and learning **FastAPI**
- Working through **CS50's Introduction to AI with Python**
- Writing cleaner, better-tested Python

---

## Certifications

- **Programming for Everybody (Getting Started with Python)** — University of Michigan · Coursera  
  Grade 99.99% · Issued December 2025  
  https://coursera.org/share/fb31ceb9795c03e531a532bc2f4acfca

- **Python Data Structures** — University of Michigan · Coursera  
  Grade 99.19% · Issued December 2025  
  https://coursera.org/share/e39d0c93c0e418ce5fed5972db467d58

- **Using Python to Access Web Data** — University of Michigan · Coursera  
  Grade 98.76% · Issued December 2025  
  https://coursera.org/share/03f1637f8d472a890005abf8e7f4c13a

- **Using Databases with Python** — University of Michigan · Coursera  
  Grade 97.88% · Issued January 2025  
  https://coursera.org/share/59cd4c3af69f94bb055c6b2e8018eb31

- **Python (Basic)** — HackerRank  
  Issued December 2025  
  https://www.hackerrank.com/certificates/7828adacf80c

---

## Featured Projects

<!--
  NOTE: The repo cards below + the Stats/Top-Languages cards under "GitHub Analytics"
  are all served by  github-readme-stats.vercel.app  — a PUBLIC instance that is
  heavily rate-limited and frequently renders as broken images.
  FIX: deploy your own instance (see setup steps), then find-and-replace
       github-readme-stats.vercel.app  ->  YOUR-APP.vercel.app
  throughout this file. It appears in 5 places (3 repo cards + 2 analytics cards).
-->

<div align="center">

<a href="https://github.com/subham-hq/python_basics">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=subham-hq&repo=python_basics&theme=tokyonight&hide_border=true" alt="python_basics" />
</a>
<a href="https://github.com/subham-hq/python_problem_solving">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=subham-hq&repo=python_problem_solving&theme=tokyonight&hide_border=true" alt="python_problem_solving" />
</a>
<a href="https://github.com/subham-hq/python_web_data_projects">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=subham-hq&repo=python_web_data_projects&theme=tokyonight&hide_border=true" alt="python_web_data_projects" />
</a>

</div>

> The cards above pull each repo's **description** straight from GitHub. Set a clear one-line description on each repository (Repo → About → ⚙️) so the cards read well — otherwise they render with the title only.

---

## GitHub Analytics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=subham-hq&show_icons=true&hide_border=true&theme=tokyonight" alt="GitHub stats" height="165" />
<img src="https://streak-stats.demolab.com?user=subham-hq&hide_border=true&theme=tokyonight" alt="GitHub streak" height="165" />

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=subham-hq&layout=compact&hide_border=true&theme=tokyonight&langs_count=8" alt="Top languages" />

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/subham-hq/subham-hq/output/github-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/subham-hq/subham-hq/output/github-snake.svg" />
  <img alt="Snake eating Subham's GitHub contribution grid" src="https://raw.githubusercontent.com/subham-hq/subham-hq/output/github-snake.svg" />
</picture>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=subham-hq&theme=tokyo-night&hide_border=true&area=true" alt="Contribution graph" />

</div>

---

## Roadmap

A focused path from solid backend engineering toward production AI:

1. **Foundations** — Python depth, DSA, clean code, Git
2. **Backend** — REST APIs (FastAPI), databases, testing, basic system design
3. **Production** — deployment, reliability, real projects shipped end to end
4. **AI/ML Systems** — applied ML and AI-powered services built on strong engineering

---

## Beyond Code

Entrepreneur by background, engineer by direction. Outside of building, I follow the Apple ecosystem and developer tooling closely, and I'm a firm believer in disciplined, continuous learning. My open-source goal is simple: contribute meaningfully to Python and backend projects, and publish work that's documented well enough to be genuinely useful to others.

---

## Connect

<p>
  <a href="mailto:subham.bh@icloud.com"><img src="https://img.shields.io/badge/Email-subham.bh@icloud.com-7AA2F7?style=flat-square&logo=icloud&logoColor=white&labelColor=1A1B27" alt="Email" /></a>
  <a href="https://www.linkedin.com/in/subham-bh"><img src="https://img.shields.io/badge/LinkedIn-subham--bh-7AA2F7?style=flat-square&logo=linkedin&logoColor=white&labelColor=1A1B27" alt="LinkedIn" /></a>
  <a href="https://github.com/subham-hq"><img src="https://img.shields.io/badge/GitHub-subham--hq-7AA2F7?style=flat-square&logo=github&logoColor=white&labelColor=1A1B27" alt="GitHub" /></a>
</p>

---

<div align="center">
<sub>Building in public — exploring the repositories is the best way to follow the progress.</sub>
</div>
